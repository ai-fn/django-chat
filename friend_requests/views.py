...
class RequestList(APIView):
    ...
    def get(request):
        ...
        accepted = RequestSerializer(requests_list_qs.filter(status=FriendRequest.Status.ACCEPTED), many=True).data
        declined = RequestSerializer(requests_list_qs.filter(status=FriendRequest.Status.DECLINED), many=True).data
        sent = RequestSerializer(requests_list_qs.filter(status=FriendRequest.Status.SENT), many=True).data
        logger.info(
...
class RequestSetAsAccepted(APIView):
    permission_classes = [IsAuthenticated, ]

    @staticmethod
    def get(request, request_pk):
...
class RequestSetAsDeclined(APIView):
    permission_classes = [IsAuthenticated, ]
