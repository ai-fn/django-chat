...
class NotificationList(APIView):
    ...
    def get(request):
        ...
        context = {
            ...
            'read': NotifSerializer(old_nofifs, many=True).data,
            'unread': NotifSerializer(new_notifs, many=True).data,
        }
    ...
    def get(request, notif_id: int):
        ...
        if notif.user == request.user:
            ...
            context = {
                ...
                'user': UserSerialize(request.user).data,
                'notif': NotifSerializer(notif).data,
            }
            notif.mark_viewed()
            return Response(data=context, status=status.HTTP_200_OK)
        else:
