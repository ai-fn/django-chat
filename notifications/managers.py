...
class NotificationQuerySet(models.QuerySet):
    ...
    def update(self, *args, **kwargs):
        """Override update to ensure cache is invalidated on every call"""
        super().update(*args, **kwargs)
        users_pks = set(self.select_related("user").values_list("user__pk", flat=True))
...
class NotificationManager(models.Manager):
    ...
    def invalidate_user_notification_cache(cls, user_pk: int) -> None:
        ...
        logger.debug("Invalidate notification cache for user with pk %s", user_pk)
